# -*- coding: utf-8 -*-
"""Emit M2.14 tests + docs + task reports (UTF-8)."""
from __future__ import annotations

from pathlib import Path

ROOT = Path(__file__).resolve().parents[1]
TESTS = ROOT / "studio-api" / "tests"
DOCS = ROOT / "docs" / "codirector" / "m2.14"
CODIR = ROOT / "docs" / "codirector"
...
TEST = r'''"""M2.14 Co-Director Unified Experience — unit/integration + hitchhiker smoke."""
from __future__ import annotations
...
REUSE_MAP = r'''# M2.14 Phase 0 — Reuse Map

| Concern | Reuse | Notes |
| --- | --- | --- |
| CoDirectorShell / Session | Extend | Three-pane UnifiedExperienceWorkspace behind flag |
| CoDirectorAttachmentTray | Reuse | Classification/interpret APIs; tray UX unchanged |
| M2.11 orchestration | Extend | SpecialistMessage + DAG nodes; no second runtime |
| M2.12 lessons | Hooks only | Emit feedback on prefs; no auto system promote |
| M2.13 VP | Optional preview | environment_preview media kind; no fake 3D |
| ProposalCard / pending approvals | Wire into Approval Center | No silent approve |
| Library / provenance | Patterns | Media cards honesty labels |
'''

DOCS_ARCH = r'''# Co-Director Unified Experience Architecture

Flag: `codirector_unified_experience_v1` / `STUDIO_FEATURE_CODIRECTOR_UNIFIED_EXPERIENCE_V1` (default OFF).

Package: `studio-api/app/codirector/m214/`. Extends M2.11 DAG/selector; does not fork orchestration.

Layout: left stage nav, center structured conversation + media rail, right collapsible inspector (plan + Approval Center).
'''

for name, body in [
    ("MEDIA_WORKSPACE.md", "# Media Workspace\n\nCards, grouping, modal expand, contextual refs, M2.13 preview slot (no fake 3D). Honesty labels: mocked / fixture / real.\n"),
    ("ATTACHMENT_INTELLIGENCE.md", "# Attachment Intelligence\n\nContent-based classify (not filename alone). Interpret proposals until confirm (approve/correct/provisional/import/merge/cancel). Secure extraction only.\n"),
    ("IDEA_TO_FINAL_WORKFLOW.md", "# Idea to Final Workflow\n\nIdea-first Storyteller discovery → format guidance → progressive depth by stage → departments → Approval Center → delivery.\n"),
    ("STORYTELLER_AGENT.md", "# Storyteller Agent\n\nFirst-class specialist. Modes: Guided / Creative / Variation. EmotionalSceneProfile + approval-aware handoff. 2–4 questions rule.\n"),
    ("EMOTIONAL_SCENE_INTELLIGENCE.md", "# Emotional Scene Intelligence\n\nEmotionalSceneProfile fields: arc, subtext, tone, stakes, character beats, unknowns, questions.\n"),
    ("SOUND_PRODUCER_AGENT.md", "# Sound Producer Agent\n\nSonicConcept; coordinates Music Supervisor / Sound Designer. No new providers. Modes Guided/Creative/Variation.\n"),
    ("INTERDEPARTMENTAL_ORCHESTRATION.md", "# Interdepartmental Orchestration\n\nSpecialistMessage bus, required exchanges (Storyteller↔Sound, Sound↔Camera/Editor, VPC→all, Continuity→all), meetings → synthesis, one primary next action.\n"),
    ("DECISION_IMPACT_AND_REVISION_PROPAGATION.md", "# Decision Impact and Revision Propagation\n\nProductionDecisionImpact calculates affected departments, revalidate keys, and preserved unaffected approvals.\n"),
    ("EVALUATION_PLAN.md", "# Evaluation Plan\n\nProjects 1–7: idea, treatment, screenplay, storyboard, sketch, partial project, complete short scene. Label mocked vs real.\n"),
    ("USABILITY.md", "# Usability\n\nOne primary next action; expandable dept detail; stage-adaptive nav; media honesty labels.\n"),
...
def main() -> None:
    w(TESTS / "test_m214_unified_experience.py", TEST)
    w(DOCS / "PHASE0_REUSE_MAP.md", REUSE_MAP)
    w(DOCS / "CODIRECTOR_UNIFIED_EXPERIENCE_ARCHITECTURE.md", DOCS_ARCH)
    docs = {
        "MEDIA_WORKSPACE.md": "# Media Workspace\n\nCards, grouping, modal expand, contextual refs, M2.13 preview slot (no fake 3D). Honesty labels: mocked / fixture / real.\n",
        "ATTACHMENT_INTELLIGENCE.md": "# Attachment Intelligence\n\nContent-based classify (not filename alone). Interpret proposals until confirm (approve/correct/provisional/import/merge/cancel). Secure extraction only.\n",
        "IDEA_TO_FINAL_WORKFLOW.md": "# Idea to Final Workflow\n\nIdea-first Storyteller discovery → format guidance → progressive depth by stage → departments → Approval Center → delivery.\n",
    ...
    for name, body in docs.items():
        w(DOCS / name, body)
    print("tests/docs emit complete")
