...
## Local runtime inventory (Phase 0 remediation)
...
| Required models (Accepted M2.7.1 image path) | `z_image_turbo_bf16.safetensors`, `qwen_3_4b.safetensors`, `ae.safetensors` — MACHINE_LOCAL_VERIFIED |
| Required nodes (Accepted M2.7.1 image path) | `UNETLoader`, `TextEncodeZImageOmni` (ComfyUI core extras) — MACHINE_LOCAL_VERIFIED |
| Other local models / custom packs | UNKNOWN_NOT_REQUIRED or MACHINE_LOCAL_UNVERIFIED for accepted image path |
