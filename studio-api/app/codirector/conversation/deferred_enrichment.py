...
def run_deferred_enrichment(
    ...
    *,
    ...
    """Sync deferred enrichment — defense-in-depth fallback.

    The sync path uses the hardened `compile_wiki_bundle` (story_compiler now
    applies the blank-when-unknown honesty rule via the write-time turn
    classifier). The async variant `run_deferred_enrichment_async` is
    preferred when the caller has a running event loop because it routes
    through the LLM-backed Story Summary Editor + per-section readiness gates.
    """
    return _run_enrichment_body(
        db,
        project_id=project_id,
        user_message=user_message,
        messages=messages,
        request_id=request_id,
        use_async_compile=False,
    )


async def run_deferred_enrichment_async(
    db: Session,
    *,
    project_id: str,
    user_message: str,
    ...
    """Async deferred enrichment — routes Wiki compile through the SAFE path.

    Uses `compile_wiki_bundle_async` so the LLM-backed Story Summary Editor
    (editor.py: edit_story_summary) runs with per-section readiness gates
    (readiness.py) + the ten hard laws (laws.py) + conservative fallback
    (fallback.py). This is the primary fix for the "Wiki populates Story
    fields with conversation material" defect: the editor's honesty rule
    leaves Logline / Short / Long blank when evidence is insufficient.

    Falls back to the hardened sync compile if the async editor raises
    (defense-in-depth — the sync path also applies the blank-when-unknown
    rule via story_compiler._is_canon_narrative).
    """
    return await _run_enrichment_body_async(
        db,
        project_id=project_id,
        user_message=user_message,
        messages=messages,
        request_id=request_id,
        provider=provider,
        use_async_compile=True,
    )


async def _run_enrichment_body_async(
    db: Session,
    *,
    project_id: str,
    user_message: str,
    ...
    request_id: str | None,
    provider: Any,
    use_async_compile: bool,
) -> dict[str, Any]:
    """Async variant of `_run_enrichment_body` that routes the compiled Wiki
    refresh through `compile_wiki_bundle_async` (the SAFE editor path).

    The read-back / verification / cache-invalidation logic mirrors the sync
    variant so the return shape is identical.
    """
    ...
    try:
        ...
        # SAFE PATH: route through compile_wiki_bundle_async so the Story Summary
        # Editor + per-section readiness gates + hard laws decide what renders.
        try:
            from ..notes.service import upsert_notes_from_texts
            from ..wiki_intelligence.compiled.page_compiler import (
                compile_wiki_bundle_async,
                compile_wiki_bundle,
            )
            ...
            if detect_explicit_wiki_write(user_message):
                ...
                try:
                    await compile_wiki_bundle_async(
                        db, project_id, force_full=False, provider=provider
                    )
                except Exception:  # noqa: BLE001
                    # Defense-in-depth: the sync path is also hardened by the
                    # write-time turn classifier + story_compiler blank-when-
                    # unknown rule (Task 3). Never silently degrade further.
                    logger.debug("async_wiki_compile_fell_back_to_sync", exc_info=True)
